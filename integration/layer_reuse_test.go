...
func testRebuildLayerReuse(t *testing.T, context spec.G, it spec.S) {
	...
	context("an app is rebuilt and icu dependency is unchanged", func() {
		it("reuses a layer from a previous build", func() {
			...
			Expect(firstImage.Buildpacks[0].Layers).To(HaveKey("icu"))
			...
			Expect(secondImage.Buildpacks[0].Layers).To(HaveKey("icu"))

			Expect(logs.String()).NotTo(ContainSubstring("  Executing build process"))
			Expect(logs.String()).To(ContainSubstring(fmt.Sprintf("  Reusing cached layer /layers/%s/icu", strings.ReplaceAll(buildpackInfo.Buildpack.ID, "/", "_"))))

			Expect(secondImage.Buildpacks[0].Layers["icu"].Metadata["built_at"]).To(Equal(firstImage.Buildpacks[0].Layers["icu"].Metadata["built_at"]))
		})
	})
}
